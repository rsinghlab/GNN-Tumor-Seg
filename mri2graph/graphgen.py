...
def discard_empty_svs(region_img,sv_features,sv_centroids,sv_labels,n_svs):
    ...
    #renumber regions
    new_regions = np.zeros(n_svs,dtype=np.int32)
    n_nodes=0
...
def img2graph(voxel_intensities,voxel_labels,approx_num_nodes=5000,boxiness=0.5,k=10):
    ...
    slic_clustering = slic(voxel_intensities,n_segments=approx_num_nodes,sigma=1,compactness=boxiness,multichannel=multi_channel,convert2lab=False)
    ...
    if(not labels_provided):
        ...
        voxel_labels=np.zeros(voxel_intensities.shape[:3])

    updated_clustering,sv_feats,sv_centroids, sv_labels = determine_node_feats_fast(copy(slic_clustering),voxel_intensities,voxel_labels,num_supervoxels)
